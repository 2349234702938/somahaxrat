...
public class OldCrystalAura
        extends Module {
    private final Timer placeTimer = new Timer();
    private final Timer breakTimer = new Timer();
    private final Timer preditTimer = new Timer();
    private final Timer manualTimer = new Timer();
    public Setting<Boolean> StopOnEating = register(new Setting<>("StopOnEating", true));
    ...
    public Setting<Float> placeRangeWall = register(new Setting<>("PlaceRangeWall", 4.0f, 0.1f, 7.0f));
    ...
    public Setting<Boolean> predicts = register(new Setting<>("Predict", true));
    ...
    public Setting<Boolean> opPlace = register(new Setting<>("1.13 Place", true));
    ...
    public Setting<Float> minArmor = register(new Setting<>("MinArmor", 4.0f, 0.1f, 80.0f));
    ...
    private final Setting<Integer> cRed = register(new Setting<Object>("OL-Red", 0, 0, 255, v -> this.outline.getValue()));
    private final Setting<Integer> cGreen = register(new Setting<Object>("OL-Green", 0, 0, 255, v -> this.outline.getValue()));
    private final Setting<Integer> cBlue = register(new Setting<Object>("OL-Blue", 255, 0, 255, v -> this.outline.getValue()));
    private final Setting<Integer> cAlpha = register(new Setting<Object>("OL-Alpha", 255, 0, 255, v -> this.outline.getValue()));
    ...
    public OldCrystalAura() {
        super("OldCrystalAura", "Oyvey Ca with only Calculation changes", Module.Category.COMBAT, true, false, false);
    }
    ...
    public void onCrystal() {
        ...
        if (StopOnEating.getValue() && isPlayerEating(mc.player)) return;
        this.realTarget = null;
        this.manualBreaker();
        int crystalCount = 0;
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue()) {
            ...
            for (BlockPos blockPos : this.placePostions(this.placeRange.getValue())) {
                if (!canSeePos(blockPos) && mc.player.getDistance(blockPos.getX(), blockPos.getY(), blockPos.getZ()) > (double) this.placeRangeWall.getValue()) continue;
                double selfDmg;
                if (blockPos == null || this.target == null || !mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(blockPos)).isEmpty() || this.target.getDistance(blockPos.getX(), blockPos.getY(), blockPos.getZ()) > (double) this.targetRange.getValue() || this.target.isDead || this.target.getHealth() + this.target.getAbsorptionAmount() <= 0.0f)
                    continue;
    ...
    private float getBlastReduction(EntityLivingBase entity, float damageI, Explosion explosion) {
        return DamageUtil.getBlastReduction(entity, damageI, explosion);
    }
